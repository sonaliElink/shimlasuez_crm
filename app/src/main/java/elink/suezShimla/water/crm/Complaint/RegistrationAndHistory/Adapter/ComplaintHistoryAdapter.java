...
public class ComplaintHistoryAdapter extends RecyclerView.Adapter<ComplaintHistoryAdapter.MyViewHolder> {
    ...
    public ComplaintHistoryAdapter(Context mCon) {
        this.mCon = mCon;
        inflater = LayoutInflater.from(mCon);
        data = new ArrayList<>();
    }

    public void addList(List<ComplaintHistoryModel> complaintHistoryModelList) {
        if (data != null) {
            clear();

            data.addAll(complaintHistoryModelList);
    ...
    @NonNull
    @Override
    public ComplaintHistoryAdapter.MyViewHolder onCreateViewHolder(@NonNull ViewGroup viewGroup, int viewType) {
        View view = inflater.inflate(R.layout.row_complaint_history, viewGroup, false);
        return new ComplaintHistoryAdapter.MyViewHolder(view);
    }

    @Override
    public void onBindViewHolder(@NonNull final ComplaintHistoryAdapter.MyViewHolder holder, int position) {
        final ComplaintHistoryModel current = data.get(position);
